...
class TestForgeConfigDistributed:
    ...
    def test_fsdp_config(self):
        cfg = ForgeConfig(**_minimal_config(
            distributed={"strategy": "fsdp", "fsdp_strategy": "hybrid_shard"}
        ))
...
class TestDeepSpeedConfigResolution:
    def test_preset_files_exist(self):
        ...
        configs_dir = os.path.join(
            os.path.dirname(__file__), "..", "configs", "deepspeed"
        )
    ...
    def test_zero2_is_stage_2(self):
        path = os.path.join(
            os.path.dirname(__file__), "..", "configs", "deepspeed", "zero2.json"
        )
        with open(path) as f:
            data = json.load(f)
    ...
    def test_zero3_is_stage_3(self):
        path = os.path.join(
            os.path.dirname(__file__), "..", "configs", "deepspeed", "zero3.json"
        )
        with open(path) as f:
            data = json.load(f)
    ...
    def test_zero3_offload_has_cpu_offload(self):
        path = os.path.join(
            os.path.dirname(__file__), "..", "configs", "deepspeed", "zero3_offload.json"
        )
        with open(path) as f:
            data = json.load(f)
        assert data["zero_optimization"]["stage"] == 3
