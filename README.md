...
- **Library API**: `from forgelm import ForgeTrainer, run_audit, run_ingest, verify_audit, verify_annex_iv, verify_gguf, gdpr_purge, reverse_pii_query, ...` — every CLI surface has a stable Python entry point under `forgelm.__all__`, version-pinned via `forgelm.__api_version__` (decoupled from `__version__`).
- **GDPR Tooling**: `forgelm purge --subject-id <id>` (Article 17 right-to-erasure: redacts data + audit-log + adapters in place, hash chain preserved via compensating entries) and `forgelm reverse-pii --query <fragment>` (Article 15 right-of-access: locates PII matches across artefacts without re-loading raw data).
