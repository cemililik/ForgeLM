...
- **Phase 12.5 polish (v0.5.3)**: `forgelm ingest --all-mask` shorthand for combined PII + secrets scrubbing; `forgelm audit --croissant` emits a [Google Croissant 1.0](http://mlcommons.org/croissant/) dataset card alongside the existing report; `forgelm audit --pii-ml` layers [Presidio](https://github.com/microsoft/presidio) NER on top of the regex detector via the optional `[ingestion-pii-ml]` extra (`person` / `organization` / `location` categories merged into the same `pii_severity` block); the wizard now offers to run `forgelm audit` automatically after a JSONL is selected (BYOD onboarding closes the audit loop).
...
pip install "forgelm[ingestion-pii-ml]"  # Presidio ML-NER for person/organization/location PII (Phase 12.5)
