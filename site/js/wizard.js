...
(function () {
  ...
  // exposes ``globalThis.WIZARD_DEFAULTS`` BEFORE this script loads;
  // ``wd(section, key, fallback)`` reads from it with a hardcoded
  // fallback so a missing JSON asset (developer running off a stale
  // checkout) doesn't break the wizard.
  function wd(section, key, fallback) {
    var defaults = globalThis.WIZARD_DEFAULTS || {};
