...
| ✅ Tamam | Site dokümantasyon düzeltme taraması | `site/*.html` üzerindeki tüm görünür YAML / artefakt-yolu / CLI / şema iddiaları artık live `forgelm/` yüzeyine karşı doğrulanıyor. Hero YAML demo'su gerçek Pydantic alan adlarıyla yeniden yazıldı, compliance artefakt ağacı disk düzenine göre yeniden çizildi, hayalet YAML anahtarları + CLI flag'leri kaldırıldı, ifadeler live davranışa hizalandı. Altı dilde i18n (en / tr / de / fr / es / zh) tam paritede (her biri 731 anahtar) |
| 🟡 Sıradaki | [v0.5.5 / v0.6.0 sürümü](roadmap/releases.md) | Yukarıdaki tüm `[Unreleased]` çalışmaları (Faz 12.6 + Faz 22 + site dokümantasyon taraması + working-memory dizin politikası) tag atılmaya hazır. Tag push `publish.yml` cross-OS matrix → PyPI'yi tetikler |
| 📋 Planlandı | [Faz 14 — Çok Aşamalı Pipeline Zincirleri](roadmap/phase-14-pipeline-chains.md) | SFT → DPO → GRPO config zinciri, pipeline kaynak izleri → `v0.6.0` (yaklaşan sürüm `v0.6.0` slot'unu alırsa `v0.7.0`) |
...
**PyPI'deki son sürüm:** `v0.5.0` — Doküman Yutma + Veri Curation Pipeline'ı (2026-04-30). `forgelm ingest`, `forgelm audit`, MinHash LSH dedup, Presidio ML-NER, Croissant 1.0 dataset card'ları, EU AI Act Madde 10 governance entegrasyonu.

**Daha öncesi:** `v0.4.5` — Quickstart Katmanı (2026-04-26); `v0.4.0` — Post-Training Tamamlama (2026-04-26).

**Sonraki (PyPI'da henüz tag'lenmemiş):** yaklaşan release üç indirilmiş kapsamı paketler: **Faz 12.6** (Library API + GDPR purge / reverse-pii + ISO 27001 / SOC 2 alignment + doctor / cache / safety-eval / verify-* subcommand'leri + cross-OS release matrix + supply-chain security baseline; [#14 webhook SSRF hardening](https://github.com/cemililik/ForgeLM/issues/14) follow-up'ını kapsar), **Faz 22** (CLI sihirbazı tarayıcı yüzeyiyle eşdeğerlik + `--wizard-start-from` ile idempotent yeniden çalıştırma + şema-güdümlü varsayılanlar SOT) ve `site/*.html` üzerindeki tüm görünür YAML / artefakt-yolu / CLI / şema iddialarını live kod yüzeyine hizalayan ve altı-dil i18n paritesini eski haline getiren bir **site dokümantasyon düzeltme taraması**.

**Güncel durum:** `development` üzerinde 19 faz (1, 2, 2.5, 3, 4, 5, 5.5, 6, 7, 8, 9, 10, 10.5, 11, 11.5, 12, 12.5, 12.6, 22) tamam. `main` `v0.5.5`'i taşıyor (merge edildi ama PyPI tag'i, post-merge çalışmaları tek tag'de toplamak için ertelendi). `CHANGELOG.md`'deki `[Unreleased]` bloğu sıradaki PyPI sürümünün hazırlık alanı; yaklaşan sürüm slot'u için durum tablosundaki **🟡 Sıradaki** satırına bakın. Sürüm sonrası 2 faz (13, 14) planlandı.
