...
| Model integrity                     | `compute_artefact_sha256`                                  | `model_integrity.json`                                  |
...
| A.8.24 Use of cryptography | FL | SHA-256 + HMAC-SHA-256 (audit chain); salted SHA-256 identifier hashing; per-output-dir salt + env XOR derivation | KMS for `FORGELM_AUDIT_SECRET` |
...
**Coverage tally for §3:** 93 controls scored. **`FL` (full)**: 11 ·
**`FL-helps`**: 50 · **`OOS`**: 32. The OOS bucket is dominated by
A.7 physical (14) + A.5 organisational governance (~10) + A.8 cloud /
network (~8) — all expected for a deployer-side toolkit boundary.
...
| CC2.3   | Communicates with regulators           | Annex IV bundle ready to ship                                    | Regulator contact list |
...
auditable evidence for ~61 of the 93 controls. The remaining 32
controls (mainly A.7 physical, A.5 organisational governance, A.8
network) are deployer-side. See §3 for the full mapping.
