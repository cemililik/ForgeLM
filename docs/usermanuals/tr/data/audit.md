...
| `--croissant` | Raporun `croissant` anahtarı altına bir [Google Croissant 1.0](http://mlcommons.org/croissant/) dataset card emit eder — aynı JSON dosyası hem audit hem Croissant-tüketicisi card görevi görür. Bkz. [Croissant 1.0 Dataset Kartı](#/data/croissant-card). |
